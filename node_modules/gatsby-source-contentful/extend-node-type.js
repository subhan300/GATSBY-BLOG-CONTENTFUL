...
const getBase64Image = imageProps => {
  ...
  const promise = new Promise((resolve, reject) => {
    base64Img.requestBase64(requestUrl, (a, b, body) => {
      ...
      fs.promises.writeFile(cacheFile, body).then(() => resolve(body)).catch(e => {
...
const resolveFixed = (image, options) => {
  ...
  const srcSet = filteredSizes.map((size, i) => {
    let resolution;
...
const resolveFluid = (image, options) => {
  ...
  if (!filteredSizes.includes(pwidth) && pwidth < CONTENTFUL_IMAGE_MAX_SIZE && Math.round(pwidth / desiredAspectRatio) < CONTENTFUL_IMAGE_MAX_SIZE) {
